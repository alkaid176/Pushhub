...
mod tests {
    ...
    // golden fixtures 直读（跨包相对路径；fixtures 的 _note 字段是
    // "未知字段忽略"语义的活体测试——守卫必须放行）。
    const PONG_FIXTURE: &str = include_str!("../../../../shared/fixtures/pong-frame.positive.json");
    ...
    fn pong_fixture_parses_with_unknown_note_field_ignored() {
        match parse_server_frame(PONG_FIXTURE) {
    ...
    #[test]
    fn history_fixture_positive_each_entry() {
        let arr: Vec<Value> = serde_json::from_str(HISTORY_POSITIVE).unwrap();
        assert!(arr.len() >= 2);
        for entry in &arr {
    ...
    fn history_fixture_negative_each_violation_drops() {
        let arr: Vec<Value> = serde_json::from_str(HISTORY_NEGATIVE).unwrap();
        assert_eq!(arr.len(), 3);
        for entry in &arr {
            let raw = serde_json::to_string(&entry["frame"]).unwrap();
            match parse_server_frame(&raw) {
                FrameResult::Drop(_) => {}
                other => panic!(
    ...
    fn unknown_type_and_unparseable_drop() {
        assert!(matches!(
            parse_server_frame(r#"{"v":1,"type":"answered","wid":"m_x"}"#),
            FrameResult::Drop(_) // 05-02 补全前的已知词汇表外类型
        ));
        assert!(matches!(
